    ...
    const fetchInterest = async () => {
      try {
        const response = await fetch(
          `${apiUrl}/interest/get/${storedUserData.user_id}`
        );
        if (!response.ok) {
      ...
      .then(async (data) => {
        ...
        if (hasCompletedBasic) {
          filtered = data?.filter(
            (fcourse) =>
              ...
              fcourse.skills
                .split(", ")
                .some((skill) => interest?.includes(skill))
          );
        ...
        const counts = await Promise.all(
          filtered.map(async (course) => {
            ...
            return {
              courseId: course.train_course_id,
              count: enrollData.count,
            };
          })
        );
  ...
  return (
    <>
      {filteredCourses.length > 0 && (
        <div id="recommend" className="container section" data-aos="fade-up">
          <br></br>
