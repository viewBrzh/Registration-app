...
function EnarollManage() {
  ...
  useEffect(() => {
    fetch("http://localhost:11230/enrollment/get-all")
      .then((response) => response.json())
      .then((data) => {
        const enrollmentData = {};
        data.forEach((enrollment) => {
          if (!enrollmentData[enrollment.train_course_id]) {
            enrollmentData[enrollment.train_course_id] = [];
          }
          enrollmentData[enrollment.train_course_id].push(enrollment);
        });
              ...
              <tbody>
                {filteredCoursesByName.map((course) => (
                  <tr key={course.train_course_id}>
                    ...
                    <td>
                      {enrollments[course.train_course_id] &&
                        enrollments[course.train_course_id].map((enrollment) => (
                          <div key={enrollment.user_id}>
